...
<nav class="fixed top-0 inset-x-0 z-50 bg-slate-950/80 backdrop-blur border-b border-white/5">
  <div class="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
    ...
    <div class="flex items-center gap-4 text-sm">
      ...
      <button x-show="!user" @click="showAuth=true" class="text-blue-400">دخول</button>
    </div>
  </div>
</nav>
  ...
  <div class="prose">
    ...
    <div x-show="!loading && activeModule>0 && !hasAccess && course.slug" class="p-8 rounded-2xl bg-blue-950/20 border border-blue-500/20 text-center my-8">
      ...
      <div class="flex flex-col sm:flex-row gap-4 justify-center max-w-md mx-auto">
        ...
        <button @click="showAuth=true" class="px-6 py-3 rounded-xl border border-white/10 hover:bg-white/5 text-sm font-semibold">تسجيل الدخول</button>
      </div>
    </div>
  </div>
</main>
